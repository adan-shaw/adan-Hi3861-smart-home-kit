...
void *KeyTask (const char *arg)
{
	...
	IoTGpioRegisterIsrFunc (HI_GPIO_IDX_0, IOT_INT_TYPE_LEVEL, IOT_GPIO_EDGE_RISE_LEVEL_HIGH, gpio_isr_func, NULL);
	...
	ret = hi_gpio_register_isr_function (HI_GPIO_IDX_0, HI_INT_TYPE_LEVEL, HI_GPIO_EDGE_FALL_LEVEL_LOW, gpio_isr_func, HI_NULL);
	if (ret != HI_ERR_SUCCESS)
	{
