...
static void FInit (BootTestExample * demo)
{
	...
	// test cases service 2
	printf ((demo < &g_example[2]) ? "[Boot Test][TaskID:%p][Step:%d][SYS Reg S:%s, F:%s]Time: %llu!\n" : "[Boot Test][TaskID:%p][Step:%d][SYSEX Reg S:%s, F:%s]Time: %llu!\n", osThreadGetId (), g_initIndex++, demo->service.GetName (&demo->service), demo->feature.GetName (&demo->feature), SAMGR_GetProcessTime ());
}
